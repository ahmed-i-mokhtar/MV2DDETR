...
def main(args):
    ...
    elif 'CarlaDataset' in args.dataset:
        base = CarlaDataset(os.path.expanduser('~/Data/CarlaDataset'))
    else:
    ...
    test_set = frameDataset(base, train=False, img_reduce=args.img_reduce, img_kernel_size=args.img_kernel_size)
    ...
    if args.resume is None:
        logdir = f'logs/{args.dataset}/{"debug_" if is_debug else ""}{"SS_" if args.semi_supervised else ""}' \
                 ...
                 f'_imgRK{args.img_reduce}_{args.img_kernel_size}_' \
    ...
    # model.load_state_dict(torch.load("/home/amokhtar/Research/Multiview/MVDeTr/logs/CarlaDataset/neck128_out0_alpha1.0_id0_drop0.0_dropcam0.0__imgRK12_10_2022-12-01_12-58-10/MultiviewDetector.pth"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', type=str, default='resnet50', choices=['vgg11', 'resnet18', 'resnet50', 'mobilenet'])
    parser.add_argument('-d', '--dataset', type=str, default='CarlaDataset', choices=['CarlaDataset','wildtrack', 'multiviewx'])
